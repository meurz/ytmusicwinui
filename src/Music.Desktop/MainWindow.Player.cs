...
public sealed partial class MainWindow
{
    ...
    private Border CreatePlayer()
    {
        ...
        var previous = IconButton("\uE892", L.Get("PreviousTrack")); previous.Click += async (_, _) => { if (playback is not null) await GuardAsync(playback.PreviousAsync); };
        ...
        var next = IconButton("\uE893", L.Get("NextTrack")); next.Click += async (_, _) => { if (playback is not null) await GuardAsync(playback.NextAsync); };
        repeatButton.Click += (_, _) => { if (playback is null) return; playback.RepeatMode = (playback.RepeatMode + 1) % 3; repeatButton.IsChecked = playback.RepeatMode != 0; ToolTipService.SetToolTip(repeatButton, new[] { L.Get("RepeatOff"), L.Get("RepeatAll"), L.Get("RepeatOne") }[playback.RepeatMode]); };
        ...
        AutomationProperties.SetName(seekSlider, L.Get("PlaybackPosition")); AutomationProperties.SetAutomationId(seekSlider, "SeekSlider");
        ...
        var volume = IconButton("\uE767", L.Get("MuteToggle")); double oldVolume = .7;
        ...
        AutomationProperties.SetName(volumeSlider, L.Get("Volume")); row.Children.Add(lyricsButton); row.Children.Add(queueButton); row.Children.Add(volume); row.Children.Add(volumeSlider); extras.Children.Add(row);
    ...
    {
        panelBody.Children.Clear(); var row = new Grid(); row.ColumnDefinitions.Add(new ColumnDefinition()); row.ColumnDefinitions.Add(new ColumnDefinition { Width = GridLength.Auto }); row.Children.Add(Text(title, 20)); var close = IconButton("\uE711", L.Get("CloseSidebar")); close.Click += (_, _) => ClosePanel(); Grid.SetColumn(close, 1); row.Children.Add(close); panelBody.Children.Add(row);
    }
    private void RenderQueue()
    {
        if (closing || sidePanel.Visibility != Visibility.Visible || panelMode != "queue") return;
        PanelHeader(L.Get("PlaybackQueue")); panelBody.Children.Add(Text(L.Get("QueueSongCount", playback?.Queue.Count ?? 0), 12, Muted));
        if (playback is null || playback.Queue.Count == 0) { panelBody.Children.Add(Text(L.Get("QueueEmpty"), 13, Muted)); return; }
    ...
    private async Task ShowLyricsAsync()
    {
        ...
        var item = playback?.Current; PanelHeader(L.Get("Lyrics"));
        if (item?.VideoId is null || core is null) { panelBody.Children.Add(Text(L.Get("SelectSongForLyrics"), 14, Muted)); return; }
        ...
        lyricText.Text = L.Get("LoadingLyrics"); lyricText.LineHeight = 35; panelBody.Children.Add(lyricText);
        try
        {
            ...
            loadedLyricsVideo = item.VideoId; lyricText.Text = JsonString(result, "text") ?? L.Get("NoLyrics");
            string? source = JsonString(result, "source"); if (!string.IsNullOrEmpty(source)) panelBody.Children.Add(Text(source, 10, Muted));
        }
        catch (OperationCanceledException) { }
        catch { if (!token.IsCancellationRequested) lyricText.Text = L.Get("LyricsUnavailable"); }
    }
