...
The client provides native navigation, search and suggestions, automatically paged library and playlist browsing, playlist creation/rename/deletion and song actions, a persistent player, queue and lyrics surfaces, and account/session settings. Feature availability also depends on the signed-in account and the core response. Lists fetch the next page as you approach the end, including search, recommendation sections and the playlist picker. Existing items, scrolling and selection are retained while results arrive. No browser is embedded for audio playback.
