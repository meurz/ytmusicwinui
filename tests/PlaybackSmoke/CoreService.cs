...
public sealed class CoreService(MusicCoreClient client, bool forceOpus, bool failFirstLoad = false) : IAsyncDisposable
{
    private int failuresRemaining = failFirstLoad ? 1 : 0;
        ...
        IProgress<MusicProgress>? progress = null)
    {
        ...
        if (parsed.RootElement.GetProperty("op").GetString() == "dash_manifest"
            && Interlocked.Exchange(ref failuresRemaining, 0) == 1)
            throw new MusicCoreException("network", "Synthetic first-load failure for retry verification.");
